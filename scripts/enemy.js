  ...
  var Enemy = /*#__PURE__*/function (_Entity) {
    ...
    _createClass(Enemy, [{
      ...
      value: function initializeStats() {
        ...
        this.baseStats.mhp = Math.floor((20 + 5 * Math.pow(this.level, 1.3) + 5 * Math.pow(1.2, this.level)) * (this.data.mhp || 1));
        this.baseStats.dmg = Math.floor((10 + 2 * Math.pow(this.level, 1.3) + 2 * Math.pow(1.2, this.level)) * (this.data.dmg || 1));
        this.baseStats.arm = Math.floor((5.33 + 3 * Math.pow(this.level, 1.3) + 3 * Math.pow(1.2, this.level)) * (this.data.arm || 1));
